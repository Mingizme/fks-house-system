  ...
  return (
    <main className="w-full max-w-[1800px] mx-auto p-6 lg:p-8 xl:p-10">
      ...
      >
              ...
              {conversations.map((c) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-medium truncate lg:text-lg">
                      {c.display_name}
                      {c.user_type === "admin" && c.admin_role && (
                        <span className="text-xs text-command font-mono ml-1 lg:text-sm">· {c.admin_role}</span>
                      )}
                    </p>
        ...
        {/* All Users section */}
        <section className="min-w-0">
          <h2 className="font-display font-bold text-lg mb-3 lg:mb-5 lg:text-2xl">{t("messages.allUsers")}</h2>
          <PlayerList
